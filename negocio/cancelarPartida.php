...
if (!is_numeric($idPartida) || (int)$idPartida <= 0) {
    echo "<script>alert('ID de partida invalido.'); window.history.back();</script>";
    exit;
}
...
if (!partidaExiste($idPartida)) {
    echo "<script>alert('No se encontro la partida especificada.'); window.history.back();</script>";
    exit;
}
...
if (isset($_POST['origen']) && $_POST['origen'] === 'dashboard') {
    echo "<script>alert('Partida eliminada correctamente'); window.location.href='../presentacion/HTML/dashboard.html';</script>";
    exit;
} else {
    echo "<script>window.location.href='../presentacion/HTML/Sala/menuSala.html';</script>";
    exit;
}
?>
